...
class TestGenerateEvents:
    ...
    async def test_only_changed_tickers_emitted(self):
        ...
        # Start a new generator, absorb initial state, then update only TSLA
        req = make_request()
    ...
    async def test_removed_ticker_pruned_from_stream(self):
        """Ticker removed from cache mid-stream must not appear in subsequent events."""
        cache = PriceCache()
        cache.update("AAPL", 190.0)
        cache.update("TSLA", 250.0)
        ...
        # Consume retry + first data event (absorbs both tickers)
        collected = []
        async with asyncio.timeout(2.0):
            async for line in gen:
                collected.append(line)
        ...
        # Remove AAPL from cache and update TSLA to trigger emission
        cache.remove("AAPL")
